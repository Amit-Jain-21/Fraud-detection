...
"""the function "reduce_memory_usage" is used to reduce the memory usage by the heavy data types in the given data frame by converting them to lighter data types without losing precision"""
...
train_identity_data = pd.read_csv('/content/drive/MyDrive/Data/train_identity.csv')
train_transaction_data = pd.read_csv('/content/drive/MyDrive/Data/train_transaction.csv')
...
"""converting the time delta given in seconds to days and hours"""
...
"""dividing columns into numeric and categorical columns in identity data

also replacing null values in categorical columns to "missing" and in numeric columns to -1 . Also visualizing some columns  
"""
...
"""dividing columns into numeric and categorical columns in transaction data
and replacing null values same as before
"""
...
"""performing some more EDA"""

sns.countplot(train_transaction_data['isFraud'])

"""counting the card types in card6 column"""

sns.countplot(train_transaction_data['card6'])

"""visualizing the fraud percentage for major card types in card6 """
...
"""exploring card types in card4 column"""

sns.countplot(train_transaction_data['card4'])

"""visualizing the fraud percentage for major card types in card4"""
...
"""finding out the minimum, maximum and average transaction amount for fraudulent and non fraudulent transaction"""
...
"""finding out minimum , maximum and average amount of transaction for all the card types available """
...
"""exploring the productCD column that has some kind of product code and finding fraud transaction counts for each associated product code"""
...
"""trying to visualize c1 to c14 columns"""
...
"""trying to visualize d1 to d15 columns"""
...
"""id_30 column has the OS version associated with the transaction and has many unique values because of the many versions of the same OS hence collapsing this column to catagorize it only according to the OS name and not according to OS version."""
...
"""device name column has the name of the device associated with the transaction and has many unique values because of the many device models of same company hence collapsing this column to catagorize it only according to the company name and not according to device model."""
...
"""id_31 column has the browser version associated with the transaction and has many unique values because of the many versions of the same browser hence collapsing this column to catagorize it only according to the browser name and not according to its versionby removing the integer part from the name."""

print(np.unique(train_identity_data['id_31']))
train_identity_data['id_31'] = train_identity_data['id_31'].str.replace('\d+', '')
print(np.unique(train_identity_data['id_31']))

"""checking if there is any change in the missing values in data after collapsing some columns"""

sns.countplot(train_identity_data['DeviceType'])

"""merging transaction data and identity data and again dividing based on numeric and categorical columns"""
...
"""using the "reduce_memory_usage" function declared above to reduce the memory usage by the merged data"""

raw_train_data = reduce_memory_usage(raw_train_data, deep=True, verbose=True)

print(raw_train_data.head(10))

"""checking if there is any change in the missing values in data after merging the data"""
...
"""looks like merging the two data tables has created a lot of missing values. Hence let us first try to identify good features from these.

checking correlation between the numeric features and removing one of the two columns that  shows more than 95% correlation. 
Finally after removing some columns selecting features that do not have null values as good features.
"""
...
Concat good categorical variables in test and train and fit using label encoder.
Transform labels to normalise encoding for test and train
Selection of columns is completely random
"""
...
train_data['card1_card2_P_emaildomain'] = train_data['card1_card2'].astype(str) + '_' + train_data['P_emaildomain'].astype(str)
train_data['P_emaildomain_addr1_card1'] = train_data['P_emaildomain_addr1'].astype(str) + '_' + train_data['card1'].astype(str)

"""exploring test data and making the same changes as done to training data"""

test_identity_data = pd.read_csv('/content/drive/MyDrive/Data/test_identity.csv')
test_transaction_data = pd.read_csv('/content/drive/MyDrive/Data/test_transaction.csv')

na_columns = test_transaction_data.isna().sum()
print(na_columns[na_columns==0])
print(na_columns[na_columns>0]/test_transaction_data.shape[0])

"""converting time delta given in seconds to hours and days, replacing null values in both transaction and identity. collapsing some columns same as done in training data."""
...
"""calling the "reduce_memory_usage" function to make the same changes to data types as done to training data"""
...
"""performing the same feature engineering as training data to make test data as needed."""
...
test_data['card1_card2_P_emaildomain'] = test_data['card1_card2'].astype(str) + '_' + test_data['P_emaildomain'].astype(str)

test_data['P_emaildomain_addr1_card1'] = test_data['P_emaildomain_addr1'].astype(str) + '_' + test_data['card1'].astype(str)

goodCategoricalVars.append('P_emaildomain_addr1_card1')
goodCategoricalVars.append('card1_card2_P_emaildomain')
...
"""using the light GBM model on the data to train the model"""
...
params = {'bagging_fraction': 0.7982116702024386,
          ...
          'n_estimators': 6200,#int(1491.3676131788188),
...
"""running the LGBM model with the parameters defined above to make prediction for the isFraud column."""
...
"""visualizing ROC curve for the 5 folds of the model implimented"""
...
"""checking the lgb score for the 5 folds"""

scores_lgb

print('Mean AUC:', np.mean(scores_lgb,axis = 0))

"""plotting the top 50 features according to their importance to better understand the features and to improve the feature engineering . we tried to merge many combinations of important features observed from here to improve our feature engineering."""
...
"""generating the submission csv with the predictions for isFraud column and uploading it to drive."""
